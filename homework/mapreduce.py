...
def _load_input(input_directory: str):
    sequence = []
...
def _save_output(output_directory: str, sequence):
    with open(f"{output_directory}/part-00000", "w", encoding="utf-8") as f:
        for key, value in sequence:
            f.write(f"{key}\t{value}\n")


def _create_marker(output_directory: str):
    with open(f"{output_directory}/_SUCCESS", "w", encoding="utf-8") as f:
        f.write("")


def run_mapreduce_job(
    mapper: Callable, reducer: Callable, input_directory: str, output_directory: str
):
